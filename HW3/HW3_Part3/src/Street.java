public class Street {
    ...
    private LDLinkedList<Building> rightBuildings;  //Building array of the buildings on the right
    private LDLinkedList<Building> leftBuildings;   //Building array of the buildings on the right

    public Street(){        //My default constructor. I initialize my fields.
        this.rightBuildings = new LDLinkedList<>();
        this.leftBuildings = new LDLinkedList<>();
        this.streetLength = 0;
    }
    ...
    /**
     * If inputs are valid, the building is added to the LDLinkedList.
     *
     * @param var The building that is being added to the LDLinkedList.
     * @return The method returns true if the building is added to the LDLinkedList, and false if the building is not added to
     * the LDLinkedList.
     */
    public boolean add(Building var){
        if(checkInputsValidity(var)){   //If inputs are valid, continues. Then, returns false.
            if(var.getPosition() == 1){
                leftBuildings.add(var);  //New element is added to LDLinkedList.
            }
            else{
    ...
    public void remove(int pos, int index) throws IndexOutOfBoundsException{
        try{
            ...
            System.out.println("Exception Handled. Entered input is invalid, please try again.");
        }
    }
